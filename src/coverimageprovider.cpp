...
QImage CoverImageProvider::requestImage(const QString &id, QSize *size, const QSize &requestedSize) {
    ...
    if (fileType.compare(QString("MP3")) == 0) {
        TagLib::MPEG::File audioFile((QString("/") + id).toUtf8().constData());
        ...
        TagLib::Ogg::Vorbis::File audioFile((QString("/") + id).toUtf8().constData());
        TagLib::Ogg::XiphComment *tag = audioFile.tag();
        if (!tag) return image;
        if (!(tag->pictureList().isEmpty())) {
            TagLib::FLAC::Picture* pic = tag->pictureList()[0];
            image.loadFromData((const unsigned char *) pic->data().data(), (int)pic->data().size());
        }
    }
